...
alias integration_test="docker-compose up -d selenium && docker-compose run --rm -e DISABLE_SLACK_CALLBACK_UPDATING=true integration && docker-compose down"
...
alias logs="docker-compose run --rm serverless logs --stage develop -f status_set --startTime ${SINCE:-5m}"
alias logs_prod="docker-compose run --rm serverless logs --stage v1 -f status_set --startTime ${SINCE:-5m}"
